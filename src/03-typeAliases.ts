...
function getBuddyListFriends(
  list: BuddyList,
  criterion: (b: Buddy) => b is Friend,
): Friend[] {
  return list.members.filter(criterion);
}

console.log(getBuddyListFriends(myBandBuddies, isFriend));

function isFriend(b: Buddy): b is Friend {
  if ("phone" in b) {
    return true;
  } else {
    return false;
